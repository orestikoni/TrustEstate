...
const baseRegisterSchema = z.object({
  ...
  phoneNumber: z
    ...
    .optional()
    .or(z.literal('')),
  role: z.enum(['Buyer', 'PropertyOwner', 'Agent', 'PropertyInspector'], {
...
export const registerSchema = baseRegisterSchema
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.role === 'Agent') {
      ...
      if (data.agencyType === 'Agency' && !data.agencyName?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Agency name is required for agency-affiliated agents',
          path: ['agencyName'],
        });
      }
    }
